<template>
	<view class="qiun-columns">
		<view class="qiun-charts">
			<canvas canvas-id="canvasPi" id="canvasPi" class="charts" :width="cWidth*pixelRatio" :height="cHeight*pixelRatio"
...
<script>
	...
	export default {
		...
		methods: {
			getServerData() {
				//接受数据
					uni.request({
						...
						for(let i =0; i<len; i++){
								let json = {};
							json.name = res.data.detail[i].deviceName;
							json.data = res.data.detail[i].battery;
							Pie.series.push(json);
						}
						
							_self.showPie("canvasPi",Pie);
						},
						fail: () => {
			...
			touchPie(e) {
				canvasPi.showToolTip(e, {
					format: function(item) {
						return item.name + ':' + item.data
					}
				});
				canvasPi.touchLegend(e, {
					animation: true
				});
			},
